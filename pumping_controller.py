...
class PumpingController:
    ...
    def __init__(self, properties: Properties, led: board.pin, pump: PumpController,
    ...
    def get_water_state(self, pumping_started, pumping_verified):
        ...
        # Once the pumping_started flag set, we stay pumping until the bottom water_level has no water
            ...
            self.debug.print_debug("UNKNOWN STATE: bottom has water %s, top has water %s, pumping_started %s"
    ...
    def check_water_level_state(self):
                ...
                if remote_cmd == self.PUMPING_CANCELED:
                    ...
                    return (
                        self.handle_remote_response(
                            ...
                            self.remote_notifier.send_pumping_canceled_ack(self.last_pump_state)))
                    ...
                    return self.handle_remote_response(
                        ...
                        self.remote_notifier.send_start_pumping_ack(self.last_pump_state))
        ...
        if self.timer.is_timed_out():
            ...
            self.stop_pumping()
            self.remote_notifier.do_error_post("TIMED OUT", "Elapsed: " + self.timer.get_elapsed())
        ...
        try:
            ...
            if not self.pumping_started_flag and water_level_state == self.IDLE:
                # There is a weird use case where the pump gets turned on at startup then doesn't turn off
                # The reservoir is full, so it needs to pump and stop (it pumps but doesn't stop).
                self.pump.pump_off()
                return self.IDLE
            ...
            # If last pump state is IDLE, and we have water, then also start pumping
            elif ((self.last_pump_state == self.IDLE or self.last_pump_state == self.READY_TO_PUMP)
                ...
                if status == self.ENGAGE_PUMP:
                    self.debug.print_debug("pumping start - starting timer")
                else:
                    self.debug.print_debug("pumping start  error "+self.error_string)
